...
/// Runs one host-side action under a grant, checking revocation first.
///
/// Returns `None`, without ever polling `action`, when the grant is already
/// revoked, and `None` when revocation arrives while the action is pending.
pub(crate) async fn unless_revoked<T>(
    grant: &mut Option<NetworkGrant>,
    action: impl std::future::Future<Output = T>,
) -> Option<T> {
    tokio::select! {
        biased;
        () = wait_for_revocation(grant) => None,
...
/// A scripted host controller for exercising controlled networking in unit tests.
#[cfg(test)]
pub(crate) mod test_support {
    ...
    pub(crate) struct TestController {
        client: NetworkControlClient,
        operations: Arc<Mutex<Vec<NetworkOperation>>>,
        allowed_flows: Arc<Mutex<Vec<u64>>>,
        outgoing: mpsc::Sender<Zeroizing<Vec<u8>>>,
        directory: PathBuf,
    ...
    impl TestController {
        ...
        pub(crate) async fn start(decision: TestDecision) -> Self {
            ...
            let host = NetworkControlHost::bind(endpoint.clone())
                ...
                .unwrap()
                .into_parts();
            let outgoing = host.outgoing.clone();
            let operations = Arc::new(Mutex::new(Vec::new()));
            let allowed_flows = Arc::new(Mutex::new(Vec::new()));
        ...
        /// The runtime-side client to hand to the network engine.
        pub(crate) fn client(&self) -> NetworkControlClient {
            self.client.clone()
        }

        /// Every operation the runtime asked about so far, in order.
        pub(crate) fn operations(&self) -> Vec<NetworkOperation> {
            self.operations.lock().unwrap().clone()
        }

        /// Wait until at least `count` operations were authorized or denied.
        pub(crate) async fn wait_for_operations(&self, count: usize) -> Vec<NetworkOperation> {
            ...
            .await
            .expect("controller should observe the expected operations")
        }

        /// Revoke every flow allowed so far.
        pub(crate) async fn revoke_all(&self) {
            let flows: Vec<u64> = self.allowed_flows.lock().unwrap().clone();
            for flow_id in flows {
                let message = ControllerMessage::Revoke { flow_id };
                self.outgoing
                    .send(Zeroizing::new(serde_json::to_vec(&message).unwrap()))
                    .await
                    .unwrap();
            }
        }
    ...
    async fn run(
        mut host: NetworkControlHostParts,
        decision: TestDecision,
        operations: Arc<Mutex<Vec<NetworkOperation>>>,
        allowed_flows: Arc<Mutex<Vec<u64>>>,
    ) {
        loop {
            let Some(bytes) = poll_fn(|context| host.incoming.poll_recv(context)).await else {
                break;
            };
            ...
            if host
                .outgoing
                .send(Zeroizing::new(serde_json::to_vec(&response).unwrap()))
                .await
                .is_err()
            {
                break;
...
mod tests {
    ...
    async fn already_revoked_grant_never_polls_the_action() {
        ...
        use test_support::{TestController, TestDecision};
        ...
        grant.as_mut().unwrap().revoked().await;

        let polled = AtomicBool::new(false);
        let outcome = unless_revoked(&mut grant, async {
        ...
        assert!(
            ...
            "a revoked grant must not run the action"
        );
    }
